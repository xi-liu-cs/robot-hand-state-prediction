...
This is state prediction given RGBD (RGB + Depth) images. The input is RGBD images of top view robot hand, after the use of several supervised learning algorithms, the output is vertex positions of each finger in meters.<br>
...
```
1. convert ```data_train``` and ```data_test``` from tensor to array
```python
