...
class CNN:
    ...
    def train_model(self, dataloader, model, loss_fn, optimizer, n_epoch):
        for e in range(n_epoch):
            ...
            for batch_inx, data in enumerate(dataloader): # for batch_inx, (x, y) in enumerate(dataloader):
                ...
                if batch_inx % 10 == 0:
                    print('Loss after mini-batch %5d: %.10f' % (batch_inx + 1, current_loss / 10))
                    current_loss = 0.0
        return model
    ...
    def main(self, loadname, pre_trained_model = None):
        ...
        model = self.train_model(dataloader=train_dataloader, model=model, loss_fn=loss_function, optimizer=optimizer, n_epoch=22)
