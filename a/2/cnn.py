...
class Data_Preprocessing:
    ...
    def tensorToArray(self, data, isTrain=True):
        ...
        if isTrain:
            return img0_array, y_array
        return img0_array, field_id_array
...
class load_images(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.isTrain == False:
            return (img0, img1, img2, depth, torch.tensor(int(field_id)))
        y = np.load(self.pathy + f + '.npy')
        return torch.tensor(np.concatenate((img0, img1, img2, depth, torch.tensor(int(field_id))))), torch.tensor(y)
        # return torch.tensor(img0, img1, img2, depth, torch.tensor(int(field_id))), torch.tensor(y)
...
class CNN:
    ...
    def main(self):
        ...
        dataset_train = load_images(train_path, isTrain = True)
        # train_dataloader = DataLoader(dataset=dataset_train, batch_size=16, shuffle=True, num_workers=2) # batchsize = 64
